...
#   timestamp: 2026-07-09T22:04:38+00:00
...
class CreateKnowledgeEntryRequest(BaseModel):
    ...
    resource: str | None = Field(
        ...
        examples=['https://console.cloud.google.com/bigquery?p=acme&d=sales&t=orders'],
    )
...
class CreateKnowledgeIndexRequest(BaseModel):
    description: str | None = Field(
        None,
        description='Human-readable description. Safe to render in user-facing messages.',
        examples=['Synced product documentation'],
    )
    ...
    source_config: Any | None = Field(
        None, description='Non-secret source coordinates. Never include credentials.'
    )
...
class CreatePluginMarketplaceRequest(BaseModel):
    name: str = Field(
        ...,
        description='Unique name within the org (kebab-case, e.g. `my-org-plugins`).',
        examples=['everruns-plugins'],
    )
    source: str = Field(
        ...,
...
class CreateTaskWebhookRequest(BaseModel):
    enabled: bool | None = Field(
        None, description='Whether the webhook starts enabled. Defaults to true.'
    )
    ...
    url: str = Field(..., description='The URL to POST task terminal-state events to.')
...
class FeatureFlagMap(RootModel[dict[str, bool]]):
...
class ForkSessionRequest(BaseModel):
    ...
    locale: str | None = Field(
        None, description='Override the locale (BCP 47).', examples=['uk-UA']
    )
    ...
    system_prompt: str | None = Field(
        None, description='Override the session-level system prompt.'
    )
    tags: list[str] | None = Field(
        None,
    ...
    title: str | None = Field(
        ...
        examples=['Branch: try the async rewrite'],
    )
...
class FormField(BaseModel):
    ...
    field_type: FieldType = Field(..., description='Input type.')
    ...
    help_text: str | None = Field(None, description='Help text shown below the input.')
    label: str = Field(..., description='Label shown next to the input.')
    name: str = Field(
        ..., description='Field name used as the key when submitting (e.g. "api_key").'
    )
    placeholder: str | None = Field(
        None, description='Placeholder text inside the input.'
    )
...
class HealthCheckCaseResult(BaseModel):
    deterministic_reason: str = Field(
        ...,
    ...
    judge_reason: str = Field(..., description='LLM judge explanation.')
    ...
    rubric: str
    score: float = Field(..., description='LLM judge score, 0.0–1.0.')
...
class InstallPluginRequest(BaseModel):
    marketplace_id: str = Field(
        ...
        examples=['plgmkt_01933b5a000070008000000000000001'],
    )
    plugin_name: str = Field(
        ...,
        description='Name of the plugin entry in the marketplace catalog.',
        examples=['microsoft-docs'],
    )
...
class KnowledgeIndexDocumentResponse(BaseModel):
    ...
    content_hash: str | None = Field(
        None, description='Content hash driving incremental re-embedding.'
    )
    created_at: AwareDatetime = Field(
        ..., description='Timestamp when this resource was created (RFC 3339).'
    )
    id: str = Field(
        ...,
        description='Prefixed public identifier. See [ID Schema](https://docs.everruns.com/advanced/id-schema/).',
        examples=['kidoc_01933b5a000070008000000000000001'],
    )
    index_id: str = Field(
        ...,
        description="Parent index's prefixed public identifier.",
        examples=['kidx_01933b5a000070008000000000000001'],
    )
    last_seen_at: AwareDatetime | None = Field(
        None,
        description='Timestamp this document was last seen during a sync pass (RFC 3339).',
    )
    mime_type: str | None = Field(None, description='Document MIME type, if known.')
    size_bytes: int | None = Field(
        None, description='Document size in bytes, if known.'
    )
    source_uri: str = Field(
        ...,
        description='Stable per-source locator (e.g. `github://owner/repo@main/docs/x.md`).',
    )
    title: str | None = Field(None, description='Document title, if known.')
    updated_at: AwareDatetime = Field(
        ..., description='Timestamp when this resource was last updated (RFC 3339).'
    )
...
class KnowledgeIndexResponse(BaseModel):
    ...
    last_sync_error: str | None = Field(
        None, description='Sanitized failure reason from the last failed sync, if any.'
    )
    last_synced_at: AwareDatetime | None = Field(
        None, description='Timestamp of the last successful sync, if any (RFC 3339).'
    )
    name: str = Field(
        ..., description='Human-readable name. Safe to render in user-facing messages.'
    )
    source_config: Any = Field(
    ...
    vector_dim: int | None = Field(
        None, description='Embedding dimension, recorded on first successful sync.'
    )
...
class ListKnowledgeIndexesQuery(BaseModel):
    include_archived: bool | None = Field(
        None,
...
class ListResponseKnowledgeIndexDocumentResponse(BaseModel):
...
class ListResponseKnowledgeIndexResponse(BaseModel):
...
class Datum17(BaseModel):
    ...
    default_provider_per_service: dict[str, str] = Field(
        ...,
...
class Datum21(BaseModel):
    created_at: AwareDatetime
    enabled: bool = Field(..., description='Whether this webhook is enabled.')
    has_secret: bool = Field(
        ...,
        description='Whether a signing secret is configured (the secret itself is never returned).',
    )
    id: str = Field(..., description='Public identifier (wh_<32-hex-chars>).')
    updated_at: AwareDatetime
    url: str = Field(..., description='Target URL that receives POST requests.')


class ListResponseTaskWebhookResponse(BaseModel):
...
class MessageBody(BaseModel):
    message: str = Field(
        ...
        examples=['Summarize the latest support tickets.'],
    )
...
class OkfFileInput(BaseModel):
    content: str = Field(
        ..., description='Raw markdown file content (frontmatter + body).'
    )
    path: str = Field(..., description='Bundle-relative path, e.g. `tables/orders.md`.')


class OkfImportSummary(BaseModel):
    created: conint(ge=0)
    pruned: conint(ge=0) = Field(
        ...,
...
class TokenUsage(BaseModel):
    ...
    effective_cost_usd: float | None = Field(
        None,
...
class ToolCallRepairedData(BaseModel):
    outcome: str = Field(
        ...,
...
class TurnSealedData(BaseModel):
    detail: str | None = Field(
        None, description='Human-readable detail for operators (optional).'
    )
    iterations: conint(ge=0) | None = Field(
        None, description='Iterations completed before the turn was sealed (if known).'
    )
    reason: str = Field(
        ...,
...
class UpdateInstalledPluginRequest(BaseModel):
    status: str | None = Field(
        None, description='New lifecycle status: `active` or `disabled`.'
    )
...
class UpdateKnowledgeEntryRequest(BaseModel):
    ...
    resource: str | None = Field(
        None, description='Optional OKF resource URI. Set to null to clear.'
    )
    tags: list[str] | None = Field(
        None,
        description='Free-form tags attached to this resource.',
...
class UpdateKnowledgeIndexRequest(BaseModel):
    description: str | None = Field(
        None, description='Human-readable description. Set to null to clear.'
    )
    embedding_model_id: str | None = Field(
        None,
...
class UpdateOrganizationRequest(BaseModel):
    ...
    default_provider_per_service: dict[str, str] | None = Field(
        None,
...
class UpdatePluginMarketplaceRequest(BaseModel):
    name: str | None = None
...
class UpdateTaskWebhookRequest(BaseModel):
    enabled: bool | None = Field(None, description='Enable or disable the webhook.')
    secret: str | None = Field(
        None,
        description='Replace the signing secret. Pass null to remove the existing secret.',
    )
    url: str | None = Field(None, description='New target URL.')
...
class WithUrlsMcpServer(BaseModel):
    ...
    protocol_mode: McpProtocolMode | None = Field(
        None,
...
class CreateHarnessRequest(BaseModel):
    ...
    embedder_metadata: dict[str, str] | None = Field(
        None,
...
class CredentialFormSchema(BaseModel):
    fields: list[FormField] = Field(..., description='Input fields to render.')
...
class DriverCredentialInfo(BaseModel):
    credential_schema: CredentialFormSchema = Field(
        ...,
        description="The fields and instructions to render for this driver's credential.",
    )
    driver: str = Field(..., description='Driver id (e.g. `openai`, `bedrock`, `mai`).')
...
class Harness(BaseModel):
    ...
    embedder_metadata: dict[str, str] | None = Field(
        ...
        examples=[{'env': 'production', 'team': 'platform'}],
    )
    id: str = Field(
        ...,
...
class HealthCheckRun(BaseModel):
    ...
    id: str = Field(..., description='Public ID (`healthcheck_…`).')
    model_id: str | None = None
    results: list[HealthCheckCaseResult] | None = Field(
        None, description='Per-case results (present once the run has produced any).'
    )
    status: HealthCheckStatus
    summary: HealthCheckSummary | None = None


class ImportOkfBundleRequest(BaseModel):
    bundle_base64: str | None = Field(
        None,
        description='A base64-encoded `.tar.gz` OKF bundle. Mutually exclusive with `files`.',
    )
    files: list[OkfFileInput] | None = Field(
        None,
...
class LatestHealthCheckRun(BaseModel):
...
class ProvidersConfigResponse(BaseModel):
    drivers: list[DriverCredentialInfo] = Field(
...
class RegenerateApiEndpointApiKeyOutput(BaseModel):
    api_key: str = Field(
        ...,
        description='New plaintext API key. Persist this — it cannot be recovered later.\nThe previous key is invalidated immediately.',
    )
    channel: AppChannel = Field(..., description='The updated api_endpoint channel.')
...
class AddApiEndpointChannelHttpRequest(BaseModel):
    auth: AppEndpointAuthConfig | None = None
    enabled: bool | None = Field(
        None, description='Whether this resource is enabled.', examples=[True]
    )
...
class AddApiEndpointChannelOutput(BaseModel):
    api_key: str = Field(
        ...,
        description='Plaintext API key. Persist this — it cannot be recovered later.',
    )
    channel: AppChannel = Field(..., description='The created api_endpoint channel.')
